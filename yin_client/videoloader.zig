...
pub fn load_video(path: []const u8, file: *const std.fs.File) !void {
    ...
    std.log.debug("Number of frames {d}", .{total_frames});
    ...
    while (ffmpeg.av_read_frame(fmt_ctx, packet) >= 0) {
        if (packet.*.stream_index == video_stream_index) {
            ...
            while (ffmpeg.avcodec_receive_frame(codec_ctx, frame) == 0) {
                ...
                std.log.info("Caching mp4: {d}%/100%...", .{@round(percentage * 100) / 100});
            }
        }
    }
}

fn iter(buffer: []u32, file: *const std.fs.File) !void {
